...
function ReviewForm({ placeId, onSubmitted }: { placeId: string; onSubmitted: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/places/${placeId}/user-reviews`, {
      method: "POST",
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-food-light rounded-2xl p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <MediaUploader folder="place-reviews" multiple max={5 - images.length} onAdd={(item) => item.type === "IMAGE" && setImages((cur) => [...cur, item.url])} />
    ...
    <div>
            ...
            <div className="bg-brand-greenBg border border-green-200 rounded-2xl p-5 flex items-center gap-4 flex-wrap">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-slate-600 mb-2">
                  Địa điểm này đã được đội ngũ admin của BookingPhanThiet.com kiểm chứng thực tế, đảm bảo chất lượng thông tin mô tả.
                </p>
                <ul className="text-sm text-slate-600 space-y-1">
                  {["Đúng địa chỉ, đúng địa điểm", "Thông tin minh bạch", "An toàn, phù hợp cho du khách"].map((t) => (
      ...
      {tab === "Đánh giá" && (
        <div className="space-y-6">
          ...
          {isLoggedIn ? (
            <ReviewForm placeId={placeId} onSubmitted={() => router.refresh()} />
          ) : (
      ...
      {tab === "Video review" && (
        <div>
          {videoId ? (
            ...
            <p className="text-sm text-food-textMuted text-center py-10">Chưa có video review cho địa điểm này.</p>
          )}
        </div>
      )}
