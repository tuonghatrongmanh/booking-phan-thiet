...
export default function PlaceReviews({ reviews, placeName }: { reviews: ReviewItem[]; placeName: string }) {
  ...
  return (
    <div className="mt-10">
      <p className="font-display font-bold text-lg text-slate-800 mb-4">Đánh giá từ du khách về {placeName}</p>
      <div className="grid sm:grid-cols-3 gap-4">
        {reviews.map((r) => (
          <div key={r.id} className="bg-white border border-slate-100 shadow-card rounded-2xl p-4">
            ...
            <p className="text-xs font-bold text-slate-500">{r.reviewerName}</p>
          </div>
        ))}
      </div>
    </div>
