...
const updateSchema = z.object({
  ...
  videoUrl: z.string().trim().regex(/^[A-Za-z0-9_-]{6,20}$/, "Video ID không hợp lệ").optional(),
...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  const permError = requireCreateOrEdit(admin, "guide-videos", "edit");
  ...
  try {
    const video = await prisma.guideVideo.update({ where: { id }, data: parsed.data });
    return NextResponse.json(video);
  } catch {
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  const { id } = await params;
  const existing = await prisma.guideVideo.findUnique({ where: { id } });
  if (!existing) return NextResponse.json({ error: "Không tìm thấy" }, { status: 404 });

  const result = await requestDeleteOrHide({
    admin,
    section: "guide-videos",
    action: "delete",
    targetType: "GuideVideo",
    targetId: id,
