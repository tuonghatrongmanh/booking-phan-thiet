...
type SP = { month?: string; type?: string };
...
export default async function BookingCalendarPage({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  const month = /^\d{4}-(0[1-9]|1[0-2])$/.test(sp.month ?? "") ? (sp.month as string) : today.slice(0, 7);
  ...
  const places = await prisma.place.findMany({
    where: { category: type === "stay" ? "HOMESTAY" : "CAR_RENTAL" },
  ...
  const tab = (t: "stay" | "car", label: string) => (
    <Link href={`/admin/lich-dat?type=${t}&month=${month}`} className={`px-4 py-2 rounded-full text-sm font-bold ${type === t ? "bg-brand-blue text-white" : "bg-white text-slate-600 border border-slate-200 hover:bg-slate-50"}`}>
      {label}
    </Link>
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        {tab("stay", "Homestay")}
        {canCar && tab("car", "Thuê xe")}
        <div className="ml-auto flex items-center gap-2">
          <Link href={`/admin/lich-dat?type=${type}&month=${shiftMonth(month, -1)}`} className="w-9 h-9 rounded-full bg-white border border-slate-200 flex items-center justify-center hover:bg-slate-50" aria-label="Tháng trước">
          ...
          <Link href={`/admin/lich-dat?type=${type}&month=${shiftMonth(month, 1)}`} className="w-9 h-9 rounded-full bg-white border border-slate-200 flex items-center justify-center hover:bg-slate-50" aria-label="Tháng sau">
            <i className="fa-solid fa-chevron-right text-xs" aria-hidden="true" />
          </Link>
          {month !== today.slice(0, 7) && (
            <Link href={`/admin/lich-dat?type=${type}`} className="text-sm font-semibold text-brand-blue hover:underline ml-1">Hôm nay</Link>
          )}
        </div>
      </div>
